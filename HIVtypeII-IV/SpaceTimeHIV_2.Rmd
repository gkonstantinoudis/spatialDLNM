...
st_typeIV_BYMmodel <- nimbleCode(
{
  ...
  for(t in 1:Temporal){gamma[t] ~ dnorm(0,tau.gamma)}
...
# Type IV spacetime interaction
  zeta[1:NT] ~ dcar_normal(adj.st[1:M], weights.st[1:M], 
                          num.st[1:NT], tau.zeta, zero_mean = 1)
...
contruct_typeIV_weights <- function(spW, ntimes) {
  ...
  # take the kronecker product of the precisions
  QtypeIV <- kronecker(Qsp,Qtm) # which is a singular thus can be seen as an ICAR
  
  # define these matrices to help make the kronecker product as the W matrix in ICAR in NIMBLE
  DtypeIV <- diag(diag(QtypeIV))
  WtypeIV <- DtypeIV - QtypeIV
  
  adj <- list(num=NULL,weights=NULL,adj=NULL)

  for (i in 1:nrow(WtypeIV)) {
